...
const elements = {
  ...
  closeDurationDialog: document.querySelector("#close-duration-dialog"),
  durationStartTime: document.querySelector("#duration-start-time"),
  durationEndTime: document.querySelector("#duration-end-time"),
  durationTotal: document.querySelector("#duration-total"),
  durationLimitStatus: document.querySelector("#duration-limit-status"),
  durationAddButtons: [...document.querySelectorAll("[data-duration-add]")],
  clearDuration: document.querySelector("#clear-duration"),
  createWithDuration: document.querySelector("#create-with-duration"),
  ...
  ntfyDialog: document.querySelector("#ntfy-dialog"),
  ntfyForm: document.querySelector("#ntfy-form"),
  ntfyEnabled: document.querySelector("#ntfy-enabled"),
  ntfyServerUrl: document.querySelector("#ntfy-server-url"),
  ntfyTopic: document.querySelector("#ntfy-topic"),
  ntfyStatus: document.querySelector("#ntfy-status"),
  ntfyError: document.querySelector("#ntfy-error"),
  closeNtfyDialog: document.querySelector("#close-ntfy-dialog"),
  saveNtfySettings: document.querySelector("#save-ntfy-settings"),
  testNtfy: document.querySelector("#test-ntfy"),
  syncNtfy: document.querySelector("#sync-ntfy"),
  runNtfyDiagnostics: document.querySelector("#run-ntfy-diagnostics"),
  ntfyDiagnostics: document.querySelector("#ntfy-diagnostics"),
  ntfyDiagnosticsStatus: document.querySelector("#ntfy-diagnostics-status"),
  ntfyDiagnosticsOutput: document.querySelector("#ntfy-diagnostics-output"),
  copyNtfyDiagnostics: document.querySelector("#copy-ntfy-diagnostics"),
  runNtfyPublishDiagnostic: document.querySelector("#run-ntfy-publish-diagnostic"),
...
const ntfyManager = new NtfySyncManager({ repository });
...
let ntfyActionInProgress = false;
let communicationDiagnosticReport = null;
let communicationDiagnosticText = "";
let publishDiagnosticAttempted = false;
...
function handleScheduleTap(event) {
  ...
  const visibleSegments = getVisibleEventSegments(events, selectedDate);
  const initialTimeRange = clockHourToAvailableTimeRange(hour, visibleSegments);
  openDurationPicker(initialTimeRange.startTime);
}
...
function appendEventSectors(fragment, assignedSegments) {
  ...
  for (const segment of assignedSegments) {
    ...
    const labelLayout = getScheduleEventLabelLayout({
    ...
    if (labelLayout.placement === "inside") {
      const labelPoint = polarPoint(labelLayout.labelRadius, labelLayout.midpoint);
      const label = createSvgElement("text", {
        x: labelPoint.x,
        y: labelPoint.y,
        dy: "0.35em",
        class: "event-label event-label--inside",
        fill: readableTextColor(segment.event.color),
        "font-size": labelLayout.fontSize,
        "text-anchor": "middle",
        "aria-hidden": "true",
      });
      label.textContent = labelLayout.text;
      fragment.append(label);
    } else {
      const leaderStart = polarPoint(outerRadius + 2, labelLayout.midpoint);
      const leaderElbow = polarPoint(248, labelLayout.midpoint);
      externalLabels.push({
        side: labelLayout.side,
        idealY: leaderElbow.y,
        leaderStart,
        leaderElbow,
        color: segment.event.color,
        text: truncateLabel(segment.event.title, 6),
      });
    }
  }

  for (const labelLayout of layoutExternalEventLabels(externalLabels)) {
    const isRight = labelLayout.side === "right";
    const textX = isRight ? 526 : 74;
    const lineEndX = isRight ? textX - 6 : textX + 6;
    const leader = createSvgElement("polyline", {
      points: [
        `${labelLayout.leaderStart.x},${labelLayout.leaderStart.y}`,
        `${labelLayout.leaderElbow.x},${labelLayout.leaderElbow.y}`,
        `${lineEndX},${labelLayout.y}`,
      ].join(" "),
      class: "event-label-leader",
      stroke: labelLayout.color,
      "aria-hidden": "true",
    });
    const label = createSvgElement("text", {
      x: textX,
      y: labelLayout.y,
      dy: "0.35em",
      class: "event-label event-label--outside",
      "font-size": 12,
      "text-anchor": isRight ? "start" : "end",
      "aria-hidden": "true",
    });
    label.textContent = labelLayout.text;
    fragment.append(leader, label);
  }
}
...
function renderDurationPicker() {
  const timeRange = calculatePlannedTimeRange(plannedStartTime, plannedDurationMinutes);
  const reachedLimit = plannedDurationMinutes >= MAX_EVENT_DURATION_MINUTES;

  elements.durationStartTime.textContent = timeRange.startTime;
  elements.durationEndTime.textContent = timeRange.endTime || "--:--";
  elements.durationTotal.textContent = formatDuration(plannedDurationMinutes);
  elements.clearDuration.disabled = plannedDurationMinutes === 0;
  elements.createWithDuration.disabled = plannedDurationMinutes === 0;
  for (const button of elements.durationAddButtons) {
    button.disabled = reachedLimit;
  }
  elements.durationLimitStatus.textContent = reachedLimit
    ? "予定時間は23時間45分が上限です。"
    : "";
  elements.durationLimitStatus.hidden = !reachedLimit;
}

function resetDurationPickerState() {
  plannedStartTime = null;
  plannedDurationMinutes = 0;
  elements.durationStartTime.textContent = "--:--";
  elements.durationEndTime.textContent = "--:--";
  elements.durationTotal.textContent = "0分";
  elements.durationLimitStatus.textContent = "";
  elements.durationLimitStatus.hidden = true;
  elements.clearDuration.disabled = true;
  elements.createWithDuration.disabled = true;
  for (const button of elements.durationAddButtons) button.disabled = false;
}

function openDurationPicker(startTime) {
  plannedStartTime = startTime;
  plannedDurationMinutes = 0;
  renderDurationPicker();
  if (typeof elements.durationDialog.showModal === "function") {
    elements.durationDialog.showModal();
  } else {
    elements.durationDialog.setAttribute("open", "");
  }
  requestAnimationFrame(() => elements.durationAddButtons[0]?.focus());
}

function closeDurationPicker() {
  if (typeof elements.durationDialog.close === "function" && elements.durationDialog.open) {
    elements.durationDialog.close();
  } else {
    elements.durationDialog.removeAttribute("open");
    resetDurationPickerState();
  }
}

function addDurationToPicker(amount) {
  const previousDuration = plannedDurationMinutes;
  plannedDurationMinutes = addPlannedDurationMinutes(plannedDurationMinutes, amount);
  renderDurationPicker();
  if (
    previousDuration < MAX_EVENT_DURATION_MINUTES &&
    previousDuration + amount > MAX_EVENT_DURATION_MINUTES
  ) {
    elements.durationLimitStatus.hidden = false;
  }
}

function clearDurationPicker() {
  plannedDurationMinutes = 0;
  renderDurationPicker();
}

function createEventFromDurationPicker() {
  if (!plannedStartTime || plannedDurationMinutes === 0) return;
  const timeRange = calculatePlannedTimeRange(plannedStartTime, plannedDurationMinutes);
  closeDurationPicker();
  openNewDialog(timeRange);
}
...
function readNtfySettingsForm() {
  return {
    enabled: elements.ntfyEnabled.checked,
    serverUrl: elements.ntfyServerUrl.value.trim(),
    topic: elements.ntfyTopic.value.trim(),
  };
}

function setNtfyError(message = "") {
  elements.ntfyError.textContent = message;
  elements.ntfyError.hidden = !message;
}

function setNtfyBusy(isBusy) {
  ntfyActionInProgress = isBusy;
  for (const button of [elements.saveNtfySettings, elements.testNtfy, elements.syncNtfy]) {
    button.disabled = isBusy;
  }
  updateDiagnosticControls();
  elements.ntfyForm.setAttribute("aria-busy", String(isBusy));
}

function updateDiagnosticControls() {
  elements.runNtfyDiagnostics.disabled = ntfyActionInProgress;
  elements.copyNtfyDiagnostics.disabled = ntfyActionInProgress || !communicationDiagnosticText;
  elements.runNtfyPublishDiagnostic.disabled =
    ntfyActionInProgress || !communicationDiagnosticReport || publishDiagnosticAttempted;
}

function renderNtfyStatus(message = "", state = "") {
  if (message) {
    elements.ntfyStatus.textContent = message;
    elements.ntfyStatus.dataset.state = state;
    return;
  }

  const { settings, scheduled, waiting, retries } = ntfyManager.getStatusSummary();
  if (!settings.enabled) {
    const lines = ["ntfy通知はOFFです。予定は端末内へ通常どおり保存されます。"];
    if (retries > 0) lines.push(`通知キャンセルの再試行待ち ${retries}件`);
    if (settings.lastError) lines.push(settings.lastError);
    elements.ntfyStatus.textContent = lines.join("\n");
    elements.ntfyStatus.dataset.state = retries > 0 ? "error" : "idle";
    return;
  }
  ...
  if (settings.lastError) lines.push(settings.lastError);
  elements.ntfyStatus.textContent = lines.join("\n");
  elements.ntfyStatus.dataset.state = retries > 0 ? "error" : waiting > 0 ? "waiting" : "success";
}

function openNtfySettingsDialog() {
  const settings = ntfyManager.getSettings();
  elements.ntfyEnabled.checked = settings.enabled;
  elements.ntfyServerUrl.value = settings.serverUrl || DEFAULT_NTFY_SERVER_URL;
  elements.ntfyTopic.value = settings.topic;
  setNtfyError();
  renderNtfyStatus();
  closeMenu({ restoreFocus: false });

  if (typeof elements.ntfyDialog.showModal === "function") {
    elements.ntfyDialog.showModal();
  } else {
    elements.ntfyDialog.setAttribute("open", "");
  }
  requestAnimationFrame(() => elements.ntfyEnabled.focus());
}

function closeNtfySettingsDialog() {
  if (ntfyActionInProgress) return;
  if (typeof elements.ntfyDialog.close === "function" && elements.ntfyDialog.open) {
    elements.ntfyDialog.close();
  } else {
    elements.ntfyDialog.removeAttribute("open");
  }
}

function queueNtfySync() {
  void ntfyManager
    .syncAll()
    .then(() => renderNtfyStatus())
    .catch(() => renderNtfyStatus("通知状態を保存できませんでした。端末の保存設定を確認してください。", "error"));
}

async function saveNtfySettings(event) {
  event.preventDefault();
  if (ntfyActionInProgress) return;
  const draft = readNtfySettingsForm();
  const validationMessage = validateNtfySettings(draft);
  if (validationMessage) {
    setNtfyError(validationMessage);
    return;
  }

  setNtfyError();
  setNtfyBusy(true);
  renderNtfyStatus("設定を保存し、通知予約を確認しています…", "waiting");
  try {
    await ntfyManager.saveSettings(draft);
    renderNtfyStatus();
    showToast("ntfy通知設定を保存しました。");
  } catch (error) {
    setNtfyError(
      error instanceof TypeError
        ? error.message
        : "通知設定を保存できませんでした。端末の保存設定を確認してください。",
    );
    renderNtfyStatus("通知設定を保存できませんでした。", "error");
  } finally {
    setNtfyBusy(false);
  }
}
...
async function sendNtfyTestNotification() {
  if (ntfyActionInProgress) return;
  const draft = readNtfySettingsForm();
  const validationMessage = validateNtfySettings({ ...draft, enabled: true });
  if (validationMessage) {
    setNtfyError(validationMessage);
    return;
  }
  ...
  setNtfyError();
  setNtfyBusy(true);
  renderNtfyStatus("テスト通知を送信しています…", "waiting");
  try {
    await ntfyManager.sendTestNotification(draft);
    renderNtfyStatus(
      "テスト通知を送信しました。ntfyアプリに届いたか確認してください。",
      "success",
    );
  } catch (error) {
    renderNtfyStatus(getNtfyTestFailureMessage(error), "error");
  } finally {
    setNtfyBusy(false);
  }
}

function renderCommunicationDiagnosticReport(message = "診断が完了しました。結果をコピーできます。") {
  communicationDiagnosticText = formatCommunicationDiagnostics(communicationDiagnosticReport);
  elements.ntfyDiagnostics.hidden = false;
  elements.ntfyDiagnosticsStatus.textContent = message;
  elements.ntfyDiagnosticsOutput.value = communicationDiagnosticText;
  updateDiagnosticControls();
  console.info(communicationDiagnosticText);
}

async function runNtfyCommunicationDiagnostics() {
  if (ntfyActionInProgress) return;
  const draft = readNtfySettingsForm();
  const validationMessage = validateNtfySettings({
    enabled: false,
  ...
  setNtfyError();
  publishDiagnosticAttempted = false;
  communicationDiagnosticReport = null;
  communicationDiagnosticText = "";
  elements.ntfyDiagnostics.hidden = false;
  elements.ntfyDiagnosticsStatus.textContent = "通信診断を実行しています…";
  elements.ntfyDiagnosticsOutput.value = "Circle Planner Communication Diagnostics\n診断中…";
  setNtfyBusy(true);

  try {
    communicationDiagnosticReport = await runCommunicationDiagnostics({
      release: document.documentElement.dataset.appRelease,
      sameOriginUrl: new URL("release.json", document.baseURI).href,
      serverUrl: draft.serverUrl,
      scope: globalThis,
      navigatorRef: navigator,
      locationRef: location,
      matchMediaImpl: window.matchMedia.bind(window),
    });
    renderCommunicationDiagnosticReport();
  } catch (error) {
    const errorName = String(error?.name || "Error");
    const errorMessage = String(error?.message || error).replace(/[\r\n\t]+/g, " ").slice(0, 500);
    elements.ntfyDiagnosticsStatus.textContent = "診断処理を開始できませんでした。";
    elements.ntfyDiagnosticsOutput.value = [
      "Circle Planner Communication Diagnostics",
      "diagnosticRunner=ERROR",
      `errorName=${errorName}`,
...
async function runNtfyPublishDiagnostics() {
  if (ntfyActionInProgress || !communicationDiagnosticReport || publishDiagnosticAttempted) return;
  const draft = readNtfySettingsForm();
  const validationMessage = validateNtfySettings({ ...draft, enabled: true });
  if (validationMessage) {
    setNtfyError(validationMessage);
  ...
  setNtfyError();
  publishDiagnosticAttempted = true;
  setNtfyBusy(true);
  elements.ntfyDiagnosticsStatus.textContent = "診断通知を1回送信しています…";
  try {
    const publishResult = await runNtfyPublishDiagnostic({
      serverUrl: draft.serverUrl,
      topic: draft.topic,
      scope: globalThis,
    });
    communicationDiagnosticReport.tests = [
      ...communicationDiagnosticReport.tests.filter((test) => test.key !== publishResult.key),
      publishResult,
    ];
    renderCommunicationDiagnosticReport(
      "通知送信診断を実行しました。診断結果とntfyアプリを確認してください。",
    );
  } finally {
    setNtfyBusy(false);
  }
...
async function copyNtfyDiagnostics() {
  if (!communicationDiagnosticText) return;
  try {
    await navigator.clipboard.writeText(communicationDiagnosticText);
    showToast("診断結果をコピーしました。");
  } catch {
    elements.ntfyDiagnosticsOutput.focus();
    elements.ntfyDiagnosticsOutput.select();
    elements.ntfyDiagnosticsOutput.setSelectionRange(0, communicationDiagnosticText.length);
    let copied = false;
    try {
    ...
    showToast(copied ? "診断結果をコピーしました。" : "診断結果を選択しました。長押ししてコピーしてください。");
  }
}

async function recheckNtfyNotifications() {
  if (ntfyActionInProgress) return;
  setNtfyError();
  setNtfyBusy(true);
  renderNtfyStatus("未予約の予定とキャンセル待ちを確認しています…", "waiting");
  try {
    await ntfyManager.syncAll();
    renderNtfyStatus();
  } catch {
    renderNtfyStatus("通知状態を確認できませんでした。現在の予定はそのまま利用できます。", "error");
  } finally {
    setNtfyBusy(false);
  }
}
...
elements.openNotificationSettings.addEventListener("click", openNtfySettingsDialog);
elements.updateApp.addEventListener("click", updateApplication);
document.addEventListener("keydown", (event) => {
  if (event.key === "Escape" && elements.durationDialog.open) {
    event.preventDefault();
    closeDurationPicker();
    return;
  }
...
elements.closeDurationDialog.addEventListener("click", closeDurationPicker);
for (const button of elements.durationAddButtons) {
  button.addEventListener("click", () => addDurationToPicker(Number(button.dataset.durationAdd)));
}
elements.clearDuration.addEventListener("click", clearDurationPicker);
elements.createWithDuration.addEventListener("click", createEventFromDurationPicker);
elements.durationDialog.addEventListener("click", (event) => {
  if (event.target === elements.durationDialog) closeDurationPicker();
});
elements.durationDialog.addEventListener("close", resetDurationPickerState);
...
elements.confirmDelete.addEventListener("click", () => {
  ...
  try {
    ...
    void Promise.resolve()
      .then(() => ntfyManager.handleEventDeleted(deletedId))
      .then(() => renderNtfyStatus())
      .catch(() => renderNtfyStatus("通知キャンセルを再試行できませんでした。予定の削除は完了しています。", "error"));
  } catch {
...
elements.ntfyForm.addEventListener("submit", saveNtfySettings);
elements.closeNtfyDialog.addEventListener("click", closeNtfySettingsDialog);
elements.testNtfy.addEventListener("click", sendNtfyTestNotification);
elements.syncNtfy.addEventListener("click", recheckNtfyNotifications);
elements.runNtfyDiagnostics.addEventListener("click", runNtfyCommunicationDiagnostics);
elements.copyNtfyDiagnostics.addEventListener("click", copyNtfyDiagnostics);
elements.runNtfyPublishDiagnostic.addEventListener("click", runNtfyPublishDiagnostics);
elements.ntfyDialog.addEventListener("click", (event) => {
  if (event.target === elements.ntfyDialog) closeNtfySettingsDialog();
});
...
document.addEventListener("visibilitychange", () => {
  if (document.visibilityState === "visible") queueNtfySync();
});
updateConnectionStatus();
...
window.setInterval(() => {
  if (document.visibilityState === "visible" && navigator.onLine) queueNtfySync();
}, 15 * 60_000);
