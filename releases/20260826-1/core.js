...
export const MAX_EVENT_DURATION_MINUTES = 23 * 60 + 45;
...
export function addPlannedDurationMinutes(
  currentDuration,
  amount,
  maximumDuration = MAX_EVENT_DURATION_MINUTES,
) {
  if (!Number.isInteger(currentDuration) || currentDuration < 0) {
    throw new RangeError(`Invalid current duration: ${currentDuration}`);
  }
  if (!Number.isInteger(amount) || amount <= 0) {
    throw new RangeError(`Invalid duration amount: ${amount}`);
  }
  if (!Number.isInteger(maximumDuration) || maximumDuration <= 0) {
    throw new RangeError(`Invalid maximum duration: ${maximumDuration}`);
  }
  return Math.min(maximumDuration, currentDuration + amount);
}

export function calculatePlannedTimeRange(startTime, durationMinutes) {
  ...
  if (
    ...
    throw new RangeError(`Invalid planned duration: ${durationMinutes}`);
  }

  if (durationMinutes === 0) {
    return { startTime, endTime: null, durationMinutes };
  }

  const endMinutes = (startMinutes + durationMinutes) % MINUTES_PER_DAY;
  return {
    startTime,
