...
export async function runCommunicationDiagnostics({
  ...
  const clientStyle = typeof extractedFetch === "function"
    ? new NtfyClient({
        fetchImpl: extractedFetch.bind(scope),
        timeoutMs,
        diagnosticLogger: () => {},
      })
...
export async function runNtfyPublishDiagnostic({
  ...
  const client = new NtfyClient({
    fetchImpl: typeof scope.fetch === "function" ? scope.fetch.bind(scope) : scope.fetch,
    timeoutMs,
    diagnosticLogger: () => {},
  });
  ...
  try {
    ...
    return {
      ...
      statusText: sanitizeValue(result.statusText),
      responseType: result.type || "unknown",
      responseOrigin: safeOrigin(result.url || serverUrl),
